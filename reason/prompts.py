...
# QARAG NOTE:
# The baseline prompt treats evidence as an unordered list of triples. QARAG can
# keep this prompt for comparability, then add a new prompt that distinguishes:
#   1. Evidence Paths       - ordered, topic-centered reading aids;
#   2. Evidence Triples     - remaining compact subgraph facts;
#   3. Question             - the original query.
# This change belongs in `prepare_prompts.py` first; prompt text should only be
# changed after the retriever starts emitting path/subgraph fields.
