...
# QARAG NOTE:
# This file is the LLM evidence serialization point. The retriever can evolve
# from top-K triples to a compact evidence subgraph without forcing changes in
# the LLM runner, as long as this file converts the retrieval result into a
...
def get_prompts(each_qa, mode, sys_prompt, cot_prompt, thres, seed=0):
    ...
    if 'rog' in mode:
        ...
        # QARAG TODO 1 - compact subgraph mode:
        # Add a new mode such as `qarag_100` or `bottleneck_100` that reads:
        #   each_qa['selected_triples']
        #   each_qa['evidence_paths']
        #   each_qa['connector_triples']
        # if these fields are emitted by `retrieve/inference.py`.
        #
        ...
        #   Question:
        #   ...
        # This keeps the final evidence as a subgraph while giving the LLM a
        # path-oriented reading order.
    ...
    # QARAG TODO 2 - prompt diagnostics:
    # Store token/triple counts here after adding compact subgraph prompting.
    # This makes it easier to report token efficiency against SubgraphRAG.
    return each_qa
